...
> 说明 使用中所有选择表名的参数 全部使用`Db::table('my_tests)` 或 `Db::name('tests)` 或 `$db->table('users)` 链式 操作
...
Db can return result in 3 different formats: Array of Array, Array of Objects and a Json string. To select a return type use ArrayBuilder(), ObjectBuilder() and JsonBuilder() methods. Note that ArrayBuilder() is a default return type
```php
// Array return type
$u= $db->getOne("users");
echo $u['login'];
...
```
    [0] => Array
        (
            ...
            [2] => Db->get() >>  file "/avb/work/PHP-MySQLi-Database-Class/tests.php" line #151
        )
    ...
    [1] => Array
        (
            ...
            [2] => Db->get() >>  file "/avb/work/PHP-MySQLi-Database-Class/tests.php" line #152
        )
