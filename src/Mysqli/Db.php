...
class Db
{
    ...
    /**
     * SELECT、INSERT、UPDATE或DELETE之后所需的SQL查询选项
     *
     * @var array
     */
    ...
    /**
     * 保存 join 连接 An array that holds where join ands
     *
     * @var array
     */
    ...
    /**
     *  在get/getone/select查询期间保存返回行数的变量
    ...
    /**
     * 变量，用于在get/getone/select查询期间使用withTotalCount（）保存返回的行数
    ...
    /**
     * 返回类型：array'将结果作为数组返回，object'作为对象
     * 'json'为json字符串
    ...
    {
        $res = $this->ArrayBuilder()->get($limit, "{$column} AS retval");
