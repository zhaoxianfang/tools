...
class Model
{
    ...
    // 获取模型类名(驼峰转下划线)
    public function underlineConvert($str)
    {
    ...
    {
        $this->db->where($this->db->getTable() . '.' . $this->primaryKey, $id);
    ...
    protected function getOne($fields = null)
    {
        ...
        $results = $this->db->ArrayBuilder()->getOne($fields);
        if ($this->db->count == 0) {
            return null;
        }
    ...
    {
        $res = $this->db->ArrayBuilder()->getValue($column, $limit);
        if (!$res) {
            return null;
        }
        return $res;
    ...
    protected function get($limit = null, $fields = null)
    {
        $objects = array();
        $this->processHasOneWith();
        $results = $this->db->ArrayBuilder()->get($limit, $fields);
        if ($this->db->count == 0) {
            return null;
        }
    ...
    private function with($objectName)
    {
        ...
        $this->_with[Db::$prefix . $objectName] = $this->relations[$objectName];
    ...
    private function join($objectName, $key = null, $joinType = 'LEFT', $primaryKey = null)
    {
        ...
        if (!strchr($key, '.')) {
            $joinStr = $this->db->getTable() . ".{$key} = " . $primaryKey;
        } else {
    ...
    {
        $res = $this->db->ArrayBuilder()->getValue("count(*)");
        if (!$res) {
            return 0;
        }
        return $res;
    ...
    private function paginate($page, $fields = null)
    {
        ...
        $objects             = array();
        $this->processHasOneWith();
        $res              = $this->db->paginate($page, $fields);
    ...
    private function validate($data)
    {
        ...
        foreach ($this->dbFields as $key => $desc) {
            ...
            if ($required && strlen($value) == 0) {
                $this->errors[] = array($this->db->getTable() . "." . $key => "is required");
                continue;
            }
            if ($value == null) {
                continue;
            ...
            if (!preg_match($regexp, $value)) {
                $this->errors[] = array($this->db->getTable() . "." . $key => "$type validation failed");
                continue;
            }
        }
