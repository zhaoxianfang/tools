...
namespace zxf\Pay\WeChat\Crypto;
...
class Rsa
{
     ...
     * @param string $type  - Either `self::KEY_TYPE_PUBLIC` or `self::KEY_TYPE_PRIVATE` string, default is
     ...
     *
     * The `\$thing` can be the `public.spki://`, `public.pkcs1://`, `private.pkcs1://`, `private.pkcs8://` protocols
     * string, eg:
    ...
    private static function parse($thing, string $type = self::KEY_TYPE_PRIVATE)
    {
        ...
        if (
            is_string($src) && is_int(strpos($src, self::PKEY_PEM_NEEDLE))
     ...
     *
     * Some of APIs were required the `$padding` mode as of `RSAES-PKCS1-v1_5` which is equal to the
     * `OPENSSL_PKCS1_PADDING` constant, exposed it for this case.
     *
     ...
     *
     * @param string                                                          $message   - Content will be
     *                                                                                   `openssl_verify`.
     * @param string                                                          $signature - The base64-encoded
     *                                                                                   ciphertext.
     * @param \OpenSSLAsymmetricKey|\OpenSSLCertificate|resource|string|mixed $publicKey - The public key.
     *
    ...
    /**
     * Decrypts base64 encoded string with `$privateKey` in the `$padding`(default is `OPENSSL_PKCS1_OAEP_PADDING`)
     * mode.
     *
     * Some of APIs were required the `$padding` mode as of `RSAES-PKCS1-v1_5` which is equal to the
     * `OPENSSL_PKCS1_PADDING` constant, exposed it for this case.
     *
     * @param string                                                                               $ciphertext - Was
     *                                                                                                         previously
     *                                                                                                         encrypted
     *                                                                                                         string
     *                                                                                                         using
     ...
     *                                                                                                         public
     *                                                                                                         key.
     * @param \OpenSSLAsymmetricKey|\OpenSSLCertificate|resource|string|array{string,string}|mixed $privateKey - The
     *                                                                                                         private
     *                                                                                                         key.
     * @param int                                                                                  $padding    - One of
     *                                                                                                         OPENSSL_PKCS1_PADDING,
