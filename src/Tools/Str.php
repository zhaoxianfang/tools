...
class Str
{
     ...
     * 生成随机字符串
     *
     * @param  string  $type  随机类型：alnum(数字+字母)、alpha(仅字符)、numeric(仅数字)、up_letter(仅大写字母)
     * @return string
     *
     * @throws Exception
     */
    public static function random(int $length = 6, string $type = 'alnum')
    {
        $charset = match ($type) {
            'alpha' => 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', // 仅字母
            'numeric' => '0123456789', // 仅数字
            'up_letter' => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', // 仅大写字母
            default => '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', // 字母和数字
        };

        return self::generateRandomString($length, $charset);
    }
