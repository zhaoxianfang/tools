 ...
 * 时区转换器
 * eg:
 *      $converter = TimeZoneConverter::instance();
 *      // 当上海时间是 2024-04-08 12:00:00 时，计算出纽约的当地时间
 *      $converter->convertToTimeZone('2024-04-08 12:00:00','Asia/Shanghai', 'America/New_York');
 *      // 计算上海和纽约的时差
 *      $converter->getTimeZoneDifference('Asia/Shanghai', 'America/New_York');
 */
class TimeZoneConverter
{
     ...
     * 添加时区差值
     *
     * @param array $timeZoneDifferences 时区差值 eg: ['Asia/Shanghai' => 8]
     *
     * @return $this
     */
     ...
     *
     * @param string $datetime     $fromTimeZone时区 时间字符串 eg: '2024-04-08 12:00:00'
     ...
     * @throws Exception
     */
    public function convertToTimeZone(string $datetime, string $fromTimeZone, string $toTimeZone, string $format = 'Y-m-d H:i:s'): string
    {
     ...
     * @throws Exception
     */
    private function getTimeZoneOfUTC(string $timeZone): mixed
    {
        if (array_key_exists($timeZone, $this->timeZoneDifferences)) {
            ...
            $dateTimeA = new DateTime('now', new DateTimeZone($timeZone));
            $dateTimeA->setTimezone(new DateTimeZone('UTC'));
            $diff = $dateTimeA->getOffset() / 3600;
        }
        return $diff;
    }
     ...
     * @throws Exception
     */
    public function getTimeZoneDifference(string $timeZoneA, string $timeZoneB): float|int
    {
        return abs($this->getTimeZoneOfUTC($timeZoneA) - $this->getTimeZoneOfUTC($timeZoneB));
    }
}
