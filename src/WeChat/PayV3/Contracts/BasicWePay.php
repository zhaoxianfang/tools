...
abstract class BasicWePay
{
    ...
    /**
     * Basiczxf\WeChat\PayV3 constructor.
     *
    ...
    public function __construct(array $options = [])
    {
        ...
        if (empty($options["mch_v3_key"])) {
            throw new Exception("Missing Config -- [mch_v3_key]");
        }
        if (empty($options["cert_private"])) {
            throw new Exception("Missing Config -- [cert_private]");
        }
        if (empty($options["cert_public"])) {
            throw new Exception("Missing Config -- [cert_public]");
        }
        ...
        if (stripos($options["cert_public"], "-----BEGIN CERTIFICATE-----") === false) {
            if (file_exists($options["cert_public"])) {
                ...
                throw new Exception("File Non-Existent -- [cert_public]");
            }
        }
        ...
        if (empty($this->config["cert_serial"])) {
            throw new Exception("Failed to parse certificate public key");
        }
    }
