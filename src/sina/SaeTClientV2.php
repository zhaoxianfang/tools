...
class SaeTClientV2
{
     ...
     * @return void
     */
    public function __construct($config = array(), $access_token, $refresh_token = null)
    {
        if (!$config || !$config['wb_akey'] || !$config['wb_skey'] || !$config['wb_callback_url']) {
            // throw new Exception("缺少必要的参数： appid、appkey、callbackUrl");
            throw new Exception('缺少必要的参数： wb_akey、wb_skey、wb_callback_url');
        }
        $this->oauth = new SaeTOAuthV2($config, $access_token, $refresh_token);
    }
