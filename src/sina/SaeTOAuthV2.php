...
class SaeTOAuthV2
{
    ...
    public function __construct($config = array(), $access_token = null, $refresh_token = null)
    {
        if (!$config['wb_akey'] || !$config['wb_skey'] || !$config['wb_callback_url']) {
            // throw new Exception("缺少必要的参数： appid、appkey、callbackUrl");
            throw new Exception('缺少必要的参数： wb_akey、wb_skey、wb_callback_url');
        }
        $this->client_id     = $config['wb_akey'];
        $this->client_secret = $config['wb_skey'];
        $this->callback_url  = $config['wb_callback_url'];
    ...
    public function sina_callback()
    {
        ...
        try {
            $config = array(
                "wb_akey"         => $this->client_id,
                "wb_skey"         => $this->client_secret,
                "wb_callback_url" => $this->callback_url,
            );

            $c        = new SaeTClientV2($config, $token['access_token']);
            $uid_get  = $c->get_uid();
            $uid      = $uid_get['uid'];
        ...
        return array(
            'user_info'      => $userInfo, // 微信用户信息
            'uid'            => $uid, // 微博uid 类似于 open_id
